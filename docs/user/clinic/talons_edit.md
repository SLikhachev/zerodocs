...
**4-Профосмотры** соответсвенно. Во всех остальных случаях необходима консультация
...
Для целей диагностики число амбулаторных посещений **1**, псещений на дому **0**.
...
- *"Исход"*: 4 (без перемен);
- *"Результат"*: 14 (динамическон наблюдение).
...
>##### Шаблоны
>
>Для ускорнения процедура заполнения талона, поля:
>
>- Неотложный, Первичный
>- Оплата, Цель, Врач, Код
>- Количество посещений
...
>
>рекомендуется заполнить в ["Шаблонах"](tmpl_edit.md). и в дальнешем применять к новому
...
вкладке неоходимо заполнить поля только раздела *"Направление"*

![Вкладка Направление](./images/naprav.png)

#### Дата направления

Поле *"Дата"* заплняется либо реальной датой направления на исследование, либо оставляйте
...
Поле *"Код МО"* заполняется которким кодом МО (3 цифры) из справочника
...
присвивается порядковый номер и вкладка разблокируется.
...
автоматически транслитирирует вводимые символы. По первым 6 символам из справчника
...
Для ускорения выбра ПМУ, определенному коду можно присвоить произвольное число -
...
и колонка *"Удалить"* с иконкой `-`. Клик по ячейке `+` увеличивает количесво на 1, по
ячейке `-` соответвенно уменьшает.

### Вкладки "Полис на дату" и "ДС"

Данные вклдки для текущей версии приложения не актуальны. **Настоятельно не рекомендуется
...
Статус закрытытого талона так же можно поменять на *"Открытый"* на этой же вкладке.
...
реестры отчетов. Нет возможности изменить статус удаленного талона с пощью интерфейса
приложения. Однако, в БД талон остается и его статус может быть изменен средствами
адмистратора системы.
